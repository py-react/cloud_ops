...
const hostnamesSchema = z.object({
...
export default function HTTPRouteHostnamesList() {
    ...
    const [initialValues, setInitialValues] = useState<any>({ name: "", type: "hostnames", namespace: selectedNamespace, config: "{}" });
    ...
    useEffect(() => {
        ...
        if (resourceType === "httproute_hostnames_profile" && profiles.length > 0) {
    ...
    const fetchProfiles = async () => {
        setLoading(true);
        try {
            const data = await (DefaultService as any).apiIntegrationKubernetesLibraryHttprouteHostnamesGet({ namespace: selectedNamespace });
    ...
    const handleDelete = async (row: any) => {
        try {
            await (DefaultService as any).apiIntegrationKubernetesLibraryHttprouteHostnamesDelete({ id: row.id });
            ...
            if (error.status === 409) {
                ...
                setConflictDialog({
                    ...
                    resourceType: "HTTPRoute Hostnames Profile",
    ...
    const steps = useMemo(() => [{
        ...
        longDescription: 'Define reusable hostname lists for your HTTPRoutes.',
        component: (props: any) => <PodProfileForm {...props} namespace={selectedNamespace} title="HTTPRoute Hostnames" />
    }], [selectedNamespace]);

    return (
        <PageLayout
            title="HTTPRoute Hostnames"
            subtitle={
                <>
                    Manage reusable hostname configurations for HTTPRoutes in <span className="text-primary font-bold">{selectedNamespace}</span>.
                </>
            }
            icon={Globe}
            actions={
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <Button variant="gradient" onClick={() => {
                        ...
                        setEditingId(null);
                        setActiveTab("config");
                        setInitialValues({ name: "", type: "hostnames", namespace: selectedNamespace, config: "{}" });
                        setDialogOpen(true);
                    }}>
        ...
        >
            <div className="flex-none grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 px-0">
                <ResourceCard
                    title="Total Hostnames Profiles"
                    count={profiles.length}
                    icon={<Globe className="w-4 h-4" />}
                    color="bg-sky-500"
                    className="border-sky-500/20 bg-sky-500/5 shadow-none hover:border-sky-500/30 transition-all text-xs"
                    isLoading={loading}
                />
            </div>
            ...
            <div className="flex-1 min-h-0 mt-4">
                <ResourceTable
                    ...
                    highlightedId={resourceType === 'httproute_hostnames_profile' ? highlightedId : null}
            ...
            <FormWizard
                name="httproute-hostnames-wizard"
                ...
                onSubmit={async (values) => {
                    try {
                        ...
                        if (editMode && editingId) {
                            await (DefaultService as any).apiIntegrationKubernetesLibraryHttprouteHostnamesPut({ id: editingId, requestBody: payload } as any);
                        } else {
                            await (DefaultService as any).apiIntegrationKubernetesLibraryHttprouteHostnamesPost({ requestBody: payload } as any);
                        }
                ...
                heading={{
                    primary: editMode ? "Edit Hostnames Profile" : "Create Hostnames Profile",
                    secondary: "Manage HTTPRoute hostnames profiles",
                    icon: Globe
                }}
            />

            <FormWizard
                name="view-httproute-hostnames-profile"
                ...
                steps={[{
                    ...
                    longDescription: 'View the details of the selected hostnames profile.',
                    component: (props: any) => initialValues ? (
                        <ProfileAdvancedConfig profile={initialValues} profileType="httproute_hostnames_profile" />
                    ) : null,
                ...
                submitLabel="Close"
                heading={{
                    primary: "Hostnames Profile Details",
