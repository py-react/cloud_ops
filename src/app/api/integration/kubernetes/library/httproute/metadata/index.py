...
from app.db_client.controllers.kubernetes_profiles.httproute_metadata_profile import (
    list_httproute_metadata_profiles,
    create_httproute_metadata_profile,
    update_httproute_metadata_profile,
    delete_httproute_metadata_profile
)
...
async def GET(request: Request, namespace: str, ids: Optional[str] = None):
    ...
    with get_session() as session:
        specs = list_httproute_metadata_profiles(session, namespace, ids=ids_list)
        return [clean_generic_profile(s.dict()) for s in specs]

async def POST(request: Request, body: K8sHTTPRouteMetadataProfile):
    with get_session() as session:
        spec = create_httproute_metadata_profile(session, body)
        return spec.dict()

async def PUT(request: Request, id: int, body: K8sHTTPRouteMetadataProfile):
    with get_session() as session:
        data = body.dict(exclude_unset=True)
        spec = update_httproute_metadata_profile(session, id, data)
        if not spec:
...
async def DELETE(request: Request, id: int):
    with get_session() as session:
        # Dependency check
        stmt = select(K8sHTTPRoute).where(K8sHTTPRoute.metadata_profile_id == id)
        ...
        success = delete_httproute_metadata_profile(session, id)
        if not success:
             return JSONResponse(status_code=404, content={"detail": "Profile not found"})
