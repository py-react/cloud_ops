...
async def POST(request: Request, body: K8sDeploymentSelectorProfile):
    with get_session() as session:
        profile = create_profile(session, body.dict())
        return profile.dict()

async def PUT(request: Request, id: int, body: K8sDeploymentSelectorProfile):
    with get_session() as session:
        data = body.dict(exclude_unset=True)
        profile = update_profile(session, id, data)
        if not profile:
            return JSONResponse(status_code=404, content={"detail": "Selector profile not found"})
        return profile.dict()
