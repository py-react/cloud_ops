...
export default function KubernetesResourcesPage() {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      ...
      setError(null);
      try {
        const data = await DefaultService.apiKubernertesResourcesGet({}) as ResourceInfo[];
        setResources(data);
      } catch (err) {
  ...
  const handleQuickViewMatchResourceSelect = (resourceType: string) => {
    navigate(
      `/new/kubernetes/${namespace}/${resourceType.toLowerCase()}`
    );
  }
  const handleAnyDeploymentsSelect = (resourceType: string) => {
    navigate(
      `/new/kubernetes/${namespace}/deployments/${resourceType.toLowerCase()}`
    );
  }
  ...
  const handleResourceSelect = (resourceType: string) => {
    ...
    if (normalizedType in resourceTypes) {
      ...
      navigate(
        `/new/kubernetes/${namespace}/resources/${normalizedType}`
      );
    }
  };
