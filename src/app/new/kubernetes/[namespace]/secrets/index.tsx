...
export default function SecretsPage() {
  ...
  const handleViewDetails = (secret: any) => {
    navigate(`/new/kubernetes/${secret.namespace}/secrets/${secret.name}`)
  };
  ...
  return (
    ...
    >
      <ResourceTable
        title="Your Secrets"
        description={`${transformedSecrets.length} Secrets found`}
        icon={<FileKeyIcon className="w-5 h-5 text-primary" />}
        columns={columns}
